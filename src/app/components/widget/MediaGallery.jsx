...
const MediaGallery = ({ mediaItems, loading }) => {
    ...
    return (
        ...
        {
            loading && <>
                <div className="flex aspect-w-16 h-[350px] md:aspect-w-4 md:aspect-h-3 overflow-hidden rounded-lg shadow-md mb-4 justify-center items-center bg-stone-200">
                </div>
                <div className="flex gap-4 py-4 justify-center overflow-x-auto">
                    {
                        loader_thumbnails.map((item, index) =>
                            <div key={`img-gallery-loader-${index}`}
                    ...
                    <div className="flex gap-4 py-4 justify-center overflow-x-auto">
                        {
                            mediaItems && mediaItems.length > 0 && mediaItems.map((item, index) =>
                                <img key={`img-gallery-${index}`} src={item.url_thumbnail} alt={`Thumbnail ${index}`}
                                    className={`size-16 sm:size-20 object-cover rounded-md cursor-pointer hover:opacity-100 transition duration-300 ${index === activeIndex ? 'opacity-100' : 'opacity-60'}`}
