...
function createAutoMenuFromBCCategories() {
  const formatCategories = bc_categories
    ...
    .map((i) => ({
      ...i,
      url: i?.url?.path.split("/").filter(Boolean).join("-"),
      path: i?.url?.path,
      menu: {
        href: i?.url?.path.split("/").filter(Boolean).join("-"),
        visible: true,
      },
      key_words: [i?.url?.path],
    }));
...
function groupInChunks(array, groupSize) {
  const result = [];
  for (let i = 0; i < array.length; i += groupSize) {
    // Slice the array from i to i + groupSize
