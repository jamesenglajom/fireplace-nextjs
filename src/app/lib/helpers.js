...
export function getCategoryIds(category_slug, categories, bc_categories){
    const category_keywords = categories.find(i=> createSlug(i.name,"-")===category_slug)?.key_words;
    const ids = category_keywords.length>0 ? bc_categories.filter(i=> category_keywords.some(keyword => i?.url?.path.includes(keyword))).map(i=> i.category_id):[];
    return ids;
}   
